...
nadya = LINE()
#nadya = LINE("ErwQwDYT4QGCeHIVOWdd./hzYkv7U8XdgFvZOg78yNq.cWyyAoQ+JyCvBftJVP/GZZL1fWnwem4C9PA6V36cX8I=")
        ...
        if op.type == 26:
            try:
                ...
                if settings["reread"] == True:
                    if msg.toType == 0:
                        nadya.log("[%s]"%(msg._from)+msg.text)
                    else:
                        nadya.log("[%s]"%(msg.to)+msg.text)
                ...
                if settings["reread"] == True:
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"] not in bl:
                            nadya.sendMessage(at,"[收回訊息者]\n%s\n[訊息內容]\n%s"%(nadya.getContact(msg_dict[msg_id]["from"]).displayName,msg_dict[msg_id]["text"]))
                        del msg_dict[msg_id]
                else:
                    pass
            except Exception as e:
...
#==============================================================================#
                    ...
                    start = time.time()
                    nadya.sendMessage(to, "檢查中...")
                    elapsed_time = time.time() - start
                    nadya.sendMessage(to,format(str(elapsed_time)))
                elif text.lower() == 'restart':
                    nadya.sendMessage(to, "重啟中")
                    time.sleep(5)
                    nadya.sendMessage(to, "重啟完成")
                    restartBot()
#==============================================================================#
...
#==============================================================================#
                    ...
                    nadya.sendMessage(to, "查詢收回開啟")
                elif text.lower() == 'reread off':
                    settings["reread"] = False
                    nadya.sendMessage(to, "查詢收回關閉")
                elif text.lower() == 'me':
...
#==============================================================================#          
                    ...
                    for a in range(k+1):
                        ...
                        nadya.sendMessage(to, "已標註 {} 人 ".format(str(len(nama))))          
                elif text.lower() == 'read on':
                    tz = pytz.timezone("Asia/Jakarta")
                    timeNow = datetime.now(tz=tz)
                    day = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday","Friday", "Saturday"]
                    hari = ["週日", "週一", "週二", "週三", "周四", "週五", "週六"]
                    ...
                    if msg.to in read['readPoint']:
                            ...
                            with open('read.json', 'w') as fp:
                                ...
                                nadya.sendMessage(msg.to,"已讀已開啟")
                    else:
                        try:
                            del read['readPoint'][msg.to]
                            del read['readMember'][msg.to]
                        ...
                        with open('read.json', 'w') as fp:
                            json.dump(read, fp, sort_keys=True, indent=4)
                            nadya.sendMessage(msg.to, "已設立已讀點:\n" + readTime)
                    ...
                    if msg.to not in read['readPoint']:
                        nadya.sendMessage(msg.to,"已讀已關閉")
                    else:
                        try:
                            del read['readPoint'][msg.to]
                            del read['readMember'][msg.to]
                            del read['readTime'][msg.to]
                              ...
                              pass
                        nadya.sendMessage(msg.to, "已刪除已讀點\n" + readTime)
                    ...
                    if msg.to in read["readPoint"]:
                        try:
                            ...
                            pass
                        nadya.sendMessage(msg.to, "已重製已讀點\n" + readTime)
                    else:
                        nadya.sendMessage(msg.to, "尚無已讀點")
                    ...
                    if receiver in read['readPoint']:
                        ...
                        nadya.sendMessage(receiver,"請先設立已讀點")
#==============================================================================#
